...
export default function BrandDefenseBanner({ items }: Props) {
  ...
  return (
    <section className="glass rounded-[32px] md:rounded-[40px] p-8 lg:p-12 relative overflow-hidden border border-red-500/10 bg-gradient-to-br from-red-950/15 via-zinc-900/30 to-zinc-950/80 shadow-[0_4px_30px_rgba(0,0,0,0.5)] hover:shadow-[0_0_40px_rgba(239,68,68,0.06)] hover:border-red-500/25 transition-all duration-500 group/banner">
      ...
      {/* 헤더 및 상태 패널 */}
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/5 pb-6 mb-8">
        <div className="flex items-center gap-4 flex-1">
          <div className="p-3 bg-red-500/10 border border-red-500/25 rounded-2xl animate-pulse">
          ...
          <div>
            ...
            <p className="text-sm md:text-base text-zinc-300 mt-2.5 font-medium">
              평소 공식 할인율이 매우 낮은 글로벌 프리미엄 브랜드(Apple, 삼성, LG, Sony, Dell 등)의 이례적인 실시간 하락 상품 감지
            </p>
          </div>
        </div>
        <div className="flex flex-col items-end gap-3 self-start md:self-auto shrink-0">
          <div className="flex items-center gap-2 px-4 py-2 bg-red-500/10 border border-red-500/20 rounded-xl text-xs md:text-sm text-red-400 font-black tracking-wider uppercase shadow-[0_0_12px_rgba(239,68,68,0.15)] animate-pulse">
            <span className="w-2 h-2 rounded-full bg-red-500" />
            <span>CRITICAL STATUS</span>
          </div>
          <Link 
            href="/products?filter=defense" 
            className="text-xs font-black text-red-400/70 hover:text-red-400 transition-colors uppercase tracking-widest flex items-center gap-1 group/more mr-1"
          >
            더 보기 
